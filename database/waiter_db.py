...
class WaiterDb(Database):
    ...
    def count_waiters(self) -> int:
        if self.connect_to_database():
            try:
                self.cursor.execute("SELECT COUNT(*) FROM waiter")
                result = self.cursor.fetchone()
                return result[0]
            except Exception as error:
                log_error(f"System user ID: {self.__account_id}. Count Waiters Error.")
                messagebox.showerror(title="Count Waiters Error", message=error)
            finally:
                self.cursor.close()
                self.connection.close()
